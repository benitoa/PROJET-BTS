	...
	while( $row = $result->fetch () ) {
		...
		while ( $donnees = $reponse->fetch () ) {
			...
			<!--Formulaire avec les champs pr�remplis-->
					<form action="stats.php" method="post" id="modif_article">
						<p>
							...
							<input type="text" name="prenom"
								...
								value="<?php echo $donnees['niveau_scolaire'];?>" /><br />
								<input type="text" name="QR_code"
								value="<?php 
								$_SESSION['id']=$donnees['QR_code'];
								echo $donnees['QR_code'];?>" /> <input
								type="submit" name="valider" value="voir" />
						</p>
					</form>
		<?php
		}
